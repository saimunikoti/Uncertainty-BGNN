...
def evaluate_test(model, test_labels, device, dataloader, loss_fcn, g):
    ...
    model.layers[1].fc_neigh.register_forward_hook(get_activation('layers[1].fc_neigh'))
    ...
    for step, (input_nodes, seeds, blocks) in enumerate(dataloader):
        ...
        with th.no_grad():
            ...
            node_emb = activation['layers[1].fc_neigh']
    ...
    return test_acc, test_loss, node_emb
...
def run(args, device, data, best_model_path):
    ...
    test_acc, test_loss, node_emb = evaluate_test(model, test_labels, device, dataloader, loss_fcn, test_g)

    print('Test acc: {:.6f} \tTess Loss: {:.6f}'.format(test_acc, test_loss))

    filepath = cnf.datapath + "\\cora_emb.npy"
    np.save(filepath, node_emb.cpu().detach().numpy())

    nodeemb = {}
    nodeemb['coraemb'] = node_emb.cpu().detach().numpy()
    filepath = cnf.datapath + "\\cora_emb.mat"
...
if __name__ == '__main__':
    ...
    argparser.add_argument('--batch-size', type=int, default=2708)
